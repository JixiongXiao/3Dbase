...
function vec3ToVec2(points) {
  if (Array.isArray(points)) {
    const results = [];
    points.forEach(point => results.push(new Vector2(point.x, point.y)));
    return results;
  } else {
    return new Vector2(points.x, points.y);
  }
}

export { getPolygonArea, isInView, isLineCross, checkLinesCross, vec2ToNum, vec3ToNum, vec3ToVec2 };
